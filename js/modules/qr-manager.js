...
class QRManager {
    ...
    createSessionData() {
        ...
        return {
            ...
            redirectUrl: 'Student.html#attendance', // URL to redirect students to attendance page
            action: 'mark_attendance' // Action to perform when QR is scanned
        };
    }

    async generateQRCode(sessionData) {
        ...
        // Build a compact payload to improve scannability
        const qrPayload = JSON.stringify({
            pin: sessionData.pin,
            sessionId: sessionData.sessionId,
            courseCode: sessionData.courseCode,
            action: 'mark_attendance',
            redirectUrl: 'Student.html#attendance'
        });

        // Prefer LAN IP when running on localhost so others on the network can scan
        let baseOrigin = window.location.origin;
        try {
            const isLocal = /^(localhost|127\.0\.0\.1)$/i.test(window.location.hostname);
            if (isLocal) {
                const hostRes = await fetch('/api/host').then(r => r.json()).catch(() => null);
                if (hostRes && hostRes.success && hostRes.baseUrl) {
                    baseOrigin = hostRes.publicUrl || hostRes.baseUrl;
                }
            }
        } catch (_) {}

        // Create a URL that external QR scanners can open
        const redirectUrl = `${baseOrigin}/qr-redirect.html?data=${encodeURIComponent(qrPayload)}`;

        // Choose a larger, responsive size to reduce visual density
        const size = Math.max(260, Math.min(360, Math.floor(window.innerWidth * 0.45)));

        // Provide contrast and padding for the QR container
        qrcodeContainer.style.padding = '12px';
        ...
        this.qrCodeInstance = new QRCode(qrcodeContainer, {
            text: redirectUrl, // Use the redirect URL instead of raw payload
            width: size,
            height: size,
            colorDark: '#000000',
            colorLight: '#ffffff',
            // Medium error correction lowers density while staying reliable
